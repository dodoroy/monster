<template>
  <div>
    <div class="tools">
      <div class="toolbar">
        <h2>Pen Color</h2>
        <span v-for="item in colors" :key="item.name"  @click="changeColor(item)"
        :class="curColor===item.name?'cur':''">
          <img :src="item.image" :alt="item.name+' pen'">
        </span>
      </div>
      <div class="toolbar">
        <h2>Pen Thickness</h2>
        <span v-for="item in thicknesses" :key="item.name" @click="changeThickness(item)"
    ...
    <div>
      <button @click="saveCanvas">Save the Canvas</button>
      <button @click="clearCanvas">Clear the Canvas</button>
    </div>

    <div :class="['imageContainer',isShowImage?'show':'']">
      <img src="" alt="" id="saveImageCopy">
